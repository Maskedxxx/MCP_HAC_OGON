# airbnb_client.py
"""
Модульный клиент для работы с Airbnb MCP сервером
"""
...
from config import MCP_SERVER_COMMAND, DEFAULT_SEARCH_PARAMS, MESSAGES, EMOJIS
...
class AirbnbMCPClient:
    ...
    def search_accommodations(self, location: str, **kwargs) -> List[Dict]:
        ...
        if "result" in response and not response.get("result", {}).get("isError", False):
            ...
            print(f"{EMOJIS['error']} {MESSAGES['search_error']}")
            return []
